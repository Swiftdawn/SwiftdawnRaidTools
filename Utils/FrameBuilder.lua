...
function FrameBuilder.CreateEmoteTriggerFrame(parentFrame, trigger, width, height, font, fontSize, onChanges, isTrigger)
    ...
    if isTrigger then
        ...
        frame.hiddenFrames.delayEditBox:SetNumeric(true)
        ...
        frame.hiddenFrames.countdownEditBox:SetNumeric(true)
...
function FrameBuilder.CreateUnitHealthTriggerFrame(parentFrame, trigger, width, height, font, fontSize, onChanges, isTrigger)
    ...
    frame:SetScript("OnMouseUp", function()
        ...
        frame.hiddenFrames:SetSize(width, isTrigger and 95 or 50)
        frame:SetBackdrop({
            bgFile = "Interface\\Addons\\SwiftdawnRaidTools\\Media\\gradient32x32.tga",
            tile = true,
            tileSize = frame:GetHeight(),
...
function FrameBuilder.CreateUnitHealthConditionFrame(parentFrame, condition, width, height, font, fontSize, onChanges)
    ...
    frame.hiddenFrames.typeEditBox:SetPoint("LEFT", frame.hiddenFrames.typeTitle, "RIGHT", 5, 0)
    frame.hiddenFrames.typeEditBox:SetAutoFocus(false)
    frame.hiddenFrames.operatorTitle = frame.hiddenFrames:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    frame.hiddenFrames.operatorTitle:SetFont(font, fontSize, "")
    ...
    frame.hiddenFrames.typeEditBox:SetScript("OnEscapePressed", cancelEditing)
    frame.hiddenFrames.typeEditBox:SetScript("OnEnterPressed", acceptChanges)
    frame.hiddenFrames.operatorEditBox:SetScript("OnEscapePressed", cancelEditing)
    frame.hiddenFrames.operatorEditBox:SetScript("OnEnterPressed", acceptChanges)
    frame.hiddenFrames.valueEditBox:SetScript("OnEscapePressed", cancelEditing)
    frame.hiddenFrames.valueEditBox:SetScript("OnEnterPressed", acceptChanges)
